"""Aggressive variant: Expanded universe momentum rotation backtest.

This variant extends the locked baseline strategy to a 5-asset universe:
- BTC/USD, ETH/USD, XRP/USD, SOL/USD, AVAX/USD

Configuration remains identical to baseline:
- Pure cross-sectional momentum (no BTC regime filter)
- Gross exposure cap of 75%
- top_n = 3 (select top 3 momentum performers from 5 assets)
- No volatility targeting

The locked baseline (3-asset universe) remains the default reference.
This variant tests increased diversification by expanding the selection pool.
"""
...
def main() -> None:
    """Load data, run backtest with expanded universe, print metrics, and save output files."""
    configure_logging()
    logger = logging.getLogger(__name__)

    # Local variant overrides
    symbols = ("BTC/USD", "ETH/USD", "XRP/USD", "SOL/USD", "AVAX/USD")
    top_n = 3  # Still select top 3 from 5-asset universe
    output_dir = Path("outputs_variant_aggressive_expanded")
    ...
    logger.info("Loading OHLCV data for %d symbols at %s", len(symbols), SETTINGS.timeframe)
    ...
    paths = write_outputs(result=result, output_dir=output_dir)
